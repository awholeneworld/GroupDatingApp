...
import gachon.termproject.me4u.Content.MatchingPostContent;
import gachon.termproject.me4u.R;
import gachon.termproject.me4u.UserInfo;
import gachon.termproject.me4u.PostImage;
...
public class MatchingWritePostActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.matching_write_post);
        ...
        if (UserInfo.getIsMan()) category = "man";
        else category = "woman";
    ...
    private void post() {
        ...
        if (imagesList.size() == 0) {// 사진이 없다면? 바로 글쓰기
            ...
            // 포스트할 내용
            postContent = new MatchingPostContent(category, userId, UserInfo.getProfileImg(), title.getText().toString(), nickname, updateTime, postId, UserInfo.getPushToken(), UserInfo.getIntroduction(), UserInfo.getLocation(), contentList, imagesUrl, locationSelected, false, false, null);

            // Firebase Realtime DB에 글 내용 올리기
            databaseReference.child("MeetingPosts/" + postId).setValue(postContent).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
            ...
            for (int i = 0; i < imagesList.size(); i++) {
                try {
                    ...
                    StorageReference imageReference = storageReference.child("MeetingImagesPosted/" + userId + "/" + postId + "/" + image.getLastPathSegment());
