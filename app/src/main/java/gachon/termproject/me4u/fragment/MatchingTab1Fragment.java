...
import gachon.termproject.me4u.activity.MatchingWritePostActivity;
import gachon.termproject.me4u.adapter.MatchingPostAdapter;
...
public class MatchingTab1Fragment extends Fragment {
    ...
    private ArrayList<String> locationSelected = new ArrayList<>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.matching_tab_teammate, container, false);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivityForResult(new Intent(getActivity(), MatchingWritePostActivity.class), 1);
            }
        });
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1) {
            if (resultCode == RESULT_OK) {
                databaseReference.orderByChild("userId").equalTo(UserInfo.getUserId()).addListenerForSingleValueEvent(postEventListener);
            }
        }
    }
