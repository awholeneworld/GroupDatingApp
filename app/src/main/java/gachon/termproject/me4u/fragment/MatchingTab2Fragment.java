...
public class MatchingTab2Fragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.matching_tab_meeting, container, false);
        ...
        if (UserInfo.getIsMan()) category = "woman";
        else category = "man";
        ...
        postEventListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    if (postContent.getIsRecruited() && !postContent.getIsMatched() && postContent.getCategory().equals(category)) { // 아직 게시글은 매칭이 안되었는데
                        if (!snapshot.child("requests/" + UserInfo.getUserId()).exists()) { // 매칭 대기 목록에 앗 내가 없네 => 그럼 이성 게시물 탭에
                            if (locationSelected.isEmpty()) // 지역조건 없으면
