...
public class MatchingTabPeopleListFragment extends Fragment {
    ...
    private ArrayList<PeopleListContent> peopleList;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.matching_tab_people_list, container, false);

        content = view.findViewById(R.id.content);
        refresher = view.findViewById(R.id.refresh_layout);

        peopleList = new ArrayList<>();
        matchingPeopleListAdapter = new MatchingPeopleListAdapter(getActivity(), peopleList);
        collectionReference = FirebaseFirestore.getInstance().collection("users");
        
        content.setLayoutManager(new LinearLayoutManager(getActivity()));
        content.setHasFixedSize(true);
        content.setAdapter(matchingPeopleListAdapter);
            ...
            if (task.isSuccessful()) {
                ...
                if (selected)
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if (!userId.equals(UserInfo.getUserId()) && snapshot.getBoolean("isMan") != UserInfo.getIsMan()) {
                            ...
                            peopleList.add(new PeopleListContent(userId, nickname, profileImg, pushToken, introduction, location));
                        }
                    }
