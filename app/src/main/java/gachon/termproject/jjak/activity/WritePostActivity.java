...
import gachon.termproject.jjak.PostImage;
...
import gachon.termproject.jjak.fragment.CommunityFreeFragment;
import gachon.termproject.jjak.fragment.CommunityReviewFragment;
import gachon.termproject.jjak.fragment.CommunityTipFragment;
import gachon.termproject.jjak.fragment.MyInfoFragment;
...
public class WritePostActivity extends AppCompatActivity {
    ...
    private void post(String category) {
        ...
        if (imagesList.size() == 0) {// 사진이 없다면? 바로 글쓰기
            ...
            // 포스트할 내용
            postContent = new PostContent(category, userId, UserInfo.getProfileImg(), title.getText().toString(), nickname, updateTime, postId, UserInfo.getPushToken(), contentList, null, UserInfo.getIntroduction(), UserInfo.getLocation());
                        ...
                        public void onComplete(@NonNull Task<Uri> task) {
                            if (task.isSuccessful()) { // URL을 포스트 내용 Class(postContent)와 DB에 업데이트
                                ...
                                if (uploadFinishCount == imagesList.size()) { // 이미지 업로드가 완료되면 글을 최종적으로 업로드
                                    ...
                                    // 포스트할 내용
                                    postContent = new PostContent(category, userId, UserInfo.getProfileImg(), title.getText().toString(), nickname, updateTime, postId, UserInfo.getPushToken(), contentList, imagesUrl, UserInfo.getIntroduction(), UserInfo.getLocation());
